    ...
    <AuthenticatedLayout> 
        <div class="max-w-7xl mx-auto mb-3 py-4 mt-20 px-10">
            <div class="max-w-7xl mx-auto py-2">
                <div class="bg-gray-700 dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg border border-gray-500 dark:border-gray-600 rounded-lg">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 bg-white dark:bg-gray-800 sm:rounded-xl shadow-md">
                <form @submit.prevent="submit" class="grid grid-cols-1 gap-4 p-4 sm:p-6 lg:p-8" enctype="multipart/form-data">
                    <input type="hidden" name="_method" value="PUT">

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <InputLabel for="scholar_name" value="ชื่อทุนการศึกษา" />
                            <TextInput id="scholar_name" type="text" class="mt-1 block w-full" v-model="form.scholar_name" autofocus />
                            <span v-if="errorMessages.scholar_name" class="text-red-600">{{ errorMessages.scholar_name }}</span>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <span v-if="errorMessages.apply_date" class="text-red-600">{{ errorMessages.apply_date }}</span>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        ></textarea>
                        <span v-if="errorMessages.description" class="text-red-600">{{ errorMessages.description }}</span>
                    </div>
                    ...
                    <div>
                        ...
                        <template v-if="props.scholarship.file">
                            <div v-if="isImage(props.scholarship.file)" class="mt-2">
                                <img :src="`/storage/files/${props.scholarship.file}`" alt="Current File" class="w-64 h-64 object-cover" />
                            </div>
                            <div v-else-if="isPDF(props.scholarship.file)" class="mt-2">
                                <iframe :src="`/storage/files/${props.scholarship.file}`" class="w-full h-64 border-none" frameborder="0"></iframe>
                            </div>
