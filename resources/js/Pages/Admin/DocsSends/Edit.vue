    ...
    <div class="max-w-7xl mx-auto py-4 mt-20 px-10">
      <div class="px-4 text-white dark:text-gray-100 flex justify-between items-center">
        ...
        <div class="flex justify-end space-x-4">
          <button @click="enableEdit" v-if="hasRole('scholar') || hasRole('intlscholar')"
            class="custom-button-warning">แก้ไข</button>
        ...
        <form @submit.prevent="handleSubmit" enctype="multipart/form-data">
          <div class="grid grid-cols-1 md:grid-cols-5 gap-6 w-full">
            ...
            <div>
              ...
              <TextInput id="fname" :readonly="!isEditable" type="text" class="mt-1 block w-full" v-model="form.fname"
                required />
            </div>
            <div>
              <InputLabel for="lname" value="นามสกุล" />
              <TextInput id="lname" :readonly="!isEditable" type="text" class="mt-1 block w-full" v-model="form.lname"
                required />
            </div>
            <div>
              <InputLabel for="date" value="วันที่"  />
              <TextInput id="date" type="date" :disabled="true"   class="mt-1 block w-full" v-model="form.date"
                required />
              <p v-if="errors.date" class="text-red-500 text-sm">{{ errors.date }}</p>
            </div>
            <div>
              <InputLabel for="years" value="ปีการศึกษา" />
              <TextInput id="years" type="text" :readonly="!isEditable" class="mt-1 block w-full" v-model="form.years"
                required />
              <p v-if="errors.years" class="text-red-500 text-sm">{{ errors.years }}</p>
            </div>
          </div>

          <div v-if="hasRole('scholar') || hasRole('intlscholar')"
            class="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mt-3">
            <div v-if="!docSend.transcripts">
              <InputLabel for="operat_docs" value="แบบบันทึกการดำเนินงาน" />
              <input id="operat_docs" type="file" class="mt-1 block w-full border-gray-300 py-1 px-2 rounded-md"
                @change="handleFileChange($event, 'operat_docs')" :disabled="!isEditable" />
            ...
            <div v-if="!docSend.operat_docs">
              ...
              <input id="transcripts" type="file" class="mt-1 block w-full border-gray-300 py-1 px-2 rounded-md"
                @change="handleFileChange($event, 'transcripts')" :disabled="!isEditable" />
          ...
          <div class="mt-6 flex items-center" v-if="showSaveButtons">
            <PrimaryButton @click="handleSubmit" :class="{ 'opacity-25': form.processing }"
              :disabled="form.processing || !isEditable" class="custom-button-success me-4">
              <span v-if="form.processing">กำลังโหลด...</span>
              <span v-else> บันทึก</span>
            </PrimaryButton>

            <PrimaryButton @click="cancelEdit" class="custom-button-danger ml-3">
              ยกเลิก
            </PrimaryButton>
          </div>
...
const handleSubmit = () => {
  if (!isEditable.value) return; // ถ้าไม่ได้อยู่ในโหมดแก้ไข จะไม่ให้ส่งข้อมูล

  errors.value = {}; // รีเซ็ต errors ก่อนการ submit
  ...
  if (Object.keys(errors.value).length > 0) {
    Swal.fire({
      ...
      text: 'มีข้อผิดพลาดในฟอร์มกรุณาตรวจสอบ',
      icon: 'warning',
  ...
  if (form.operat_docs) {
    formData.append('operat_docs', form.operat_docs);
  }

  if (form.transcripts) {
    formData.append('transcripts', form.transcripts);
  }

  form.post(route('docs_sends.update', { id: props.docSend.id }), {
    data: formData,
    headers: {
    ...
    onSuccess: () => {
      Swal.fire({
        ...
        confirmButtonText: 'ตกลง',
        confirmButtonColor: '#3085d6' // สีของปุ่ม "ตกลง"
      });
...
const cancelEdit = () => {
  ...
  Swal.fire({
    ...
    text: 'ข้อมูลที่ยังไม่ได้บันทึกจะถูกลบ',
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      ...
      form.title = props.docSend.title;
      form.fname = props.docSend.fname;
      form.lname = props.docSend.lname;
      form.date = props.docSend.date;
      form.years = props.docSend.years;
      form.operat_docs = null;
      form.transcripts = null;
    }
  });
};
