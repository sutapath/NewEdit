        ...
        <div class="mt-4 flex items-center space-x-4">
          <div class="flex-1">
            ...
            <select v-model="form.document_type" id="docType"
              class="mt-1 block w-full p-2 border border-gray-300 rounded-md text-md" disabled>
...
moment.locale('th');
const { props } = usePage({});
console.log("documentRequest", props.documentRequest)
const form = useForm({
  user_id: props.auth.user.id,
  document_type: props.documentRequest?.document_type || 'transcripts',  // ตั้งค่า document_type จาก props
  send_date: props.documentRequest?.send_date ? moment(props.documentRequest.send_date).format('YYYY-MM-DD') : '',  // ตั้งค่า send_date จาก props
  type: '1'
});
  ...
  form.post(route('publish_requests.store'), {
    onSuccess: () => {
      ...
      Swal.fire({
        ...
        window.location.href = route('docs_sends.index');
      });
    },
    onError: () => {
      Swal.fire({
