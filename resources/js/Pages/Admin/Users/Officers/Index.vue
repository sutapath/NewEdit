...
import Edit from "@/Pages/Admin/Users/Officers/Edit.vue"; // แก้ไขตรงนี้เป็น Officers
import Modal from "@/Components/Modal.vue";
const { hasRole } = usePermission();
const props = defineProps({
  officers: { // เปลี่ยนจาก members เป็น officers
    type: Array,
    default: () => [],
  },
});
...
const totalItems = ref(props.officers.length); // เปลี่ยนจาก members เป็น officers

const sortedMembers = computed(() => {
  let filteredOfficers = [...props.officers]; // เปลี่ยนจาก members เป็น officers

  if (searchQuery.value) {
    filteredOfficers = filteredOfficers.filter(officer =>
      officer.citizen_id && officer.citizen_id.toString().toLowerCase().includes(searchQuery.value.toLowerCase())
    );
  }

  filteredOfficers.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

  // Pagination
  totalItems.value = filteredOfficers.length;
  ...
  return filteredOfficers.slice(start, start + perPage.value);
});

// ฟังก์ชันสำหรับลบผู้ใช้
function deleteUser(officer) { // เปลี่ยนจาก user เป็น officer
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route("officers.destroy", officer.id), { // เปลี่ยนจาก members เป็น officers
        onSuccess: () => {
          Swal.fire({
            icon: 'success',
...
function openEditModal(officer) { // เปลี่ยนจาก user เป็น officer
  userToEdit.value = officer;
...
<template>
  ...
  <Head title="Officers Index" /> <!-- เปลี่ยนจาก Users เป็น Officers -->
  ...
  <AuthenticatedLayout class="mtb">
    <div class="max-w-7xl mx-auto mb-3 py-4 mt-20 px-10">
      <div class="py-2 px-4 text-white dark:text-gray-100 flex justify-between items-center">
        ...
        <div class="flex space-x-4 mb-4 ml-3">
          <Link :href="route('officers.create')" class="custom-button-success">
          เพิ่ม
          </Link>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 bg-white sm:rounded-xl mb-20">
      <Table class="w-full overflow-x-auto">
        <template #header>
          <TableRow>
            ...
            <TableHeaderCell>รหัสเจ้าหน้าที่</TableHeaderCell>
            <TableHeaderCell>เพิ่มเติม</TableHeaderCell>
          </TableRow>
        </template>

        <template #default>
          <TableRow v-for="(officer, index) in sortedMembers" :key="officer.id" class="border-b">
            <!-- เปลี่ยนจาก member เป็น officer -->
            <TableDataCell>{{ index + 1 }}</TableDataCell>
            <TableDataCell class="table-cell-nowrap">{{ officer.citizen_id }}</TableDataCell>
            <!-- เปลี่ยนจาก member เป็น officer -->
            <TableDataCell class="table-cell-nowrap">
              {{ getTitle(officer.title) }} {{ officer.fname }} {{ officer.lname }}
              <!-- เปลี่ยนจาก member เป็น officer -->
            </TableDataCell>
            <TableDataCell class="table-cell-nowrap">{{ officer.code }}</TableDataCell>

            <TableDataCell>
              <div class="table-cell-icons">
                <button @click="openEditModal(officer)"
                ...
                <button @click="deleteUser(officer)"
      ...
      <Modal v-model:show="showEditUserModal">
        <Edit v-if="showEditUserModal" :officer="userToEdit" /> <!-- เปลี่ยนจาก user เป็น officer -->
      </Modal>
    </div>
  </AuthenticatedLayout>
</template>
