...
import Edit from "@/Pages/Admin/Users/IntlScholars/Edit.vue";
...
console.debug(props.intlScholars)
...
const totalItems = ref(props.intlScholars.length);

const sortedStudents = computed(() => {
  let filteredStudents = [...props.intlScholars];
...
function deleteStudent(student) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route("intlScholars.destroy", student.id), {
        onSuccess: () => {
          Swal.fire({
            icon: 'success',
    ...
    <div class="max-w-7xl mx-auto mb-3 py-4 mt-20 px-10">
      <div class="py-2 px-4 text-white dark:text-gray-100 flex justify-between items-center">
        ...
        <div class="flex space-x-4 mb-4 ml-3">
          <Link :href="route('intlScholars.create')" class="custom-button-success">
          เพิ่ม
          </Link>
      ...
      <Table class="w-full overflow-x-auto">
        ...
        <template #default>
          <TableRow v-for="(student, index) in sortedStudents" :key="student.id" class="border-b">
            ...
            <TableDataCell class="table-cell-nowrap">{{ student.code }}</TableDataCell>
            <TableDataCell>
