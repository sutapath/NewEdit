...
function confirmCancel() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href = route('students.index'); // กลับไปหน้าหลัก
    }
  });
}
  ...
  <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 bg-white sm:rounded-xl mb-20">
    <form @submit.prevent="submit">
      <div class="p-6 space-y-4">
        ...
        <div>
          <label for="student_code" class="block text-sm font-medium text-gray-700">รหัสนักศึกษา</label>
          <TextInput id="student_code" v-model="form.student_code" type="text" placeholder="กรุณาระบุรหัสนักศึกษา"
            class="mt-1 block w-full" @input="clearError('student_code')" />
          <span v-if="form.errors.student_code" class="text-red-500 text-sm">{{ form.errors.student_code }}</span>
        </div>
